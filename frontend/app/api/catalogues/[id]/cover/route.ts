...
const BUCKET = 'catalogue-covers';
const MAX_BYTES = 5 * 1024 * 1024;
const ALLOWED_MIME = ['image/jpeg', 'image/png', 'image/webp'];

type RouteContext = { params: Promise<{ id: string }> };

// ─── POST /api/catalogues/[id]/cover ─────────────────────────────────────────
...
export async function POST(
  ...
  if (file.size > MAX_BYTES) {
    return NextResponse.json(
      { data: null, error: 'Fichier trop lourd (max 5 MB).', success: false },
      { status: 400 },
    );
  }
  ...
  if (existing.cover_path && existing.cover_path !== storagePath) {
    await supabaseAdmin.storage.from(BUCKET).remove([existing.cover_path]);
  }
...
// ─── DELETE /api/catalogues/[id]/cover ───────────────────────────────────────
